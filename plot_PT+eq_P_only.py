...
rainfile = "rain2.csv"
kigamfile = "A2_event_list_park.csv"
...
kigam = pd.read_csv(kigamfile, header=0, names=["Time"])
kTime = [f"{kigam.loc()[row][0][0:4]}-{kigam.loc()[row][0][4:6]}-{kigam.loc()[row][0][6:8]} {kigam.loc()[row][0][9:11]}:{kigam.loc()[row][0][11:13]}:{kigam.loc()[row][0][13:15]}" for row in range (0,len(kigam),1)]
def change(tt):
    return datetime.datetime.strptime(tt, '%Y-%m-%d %H:%M:%S')
kigam.Time = list(map(change, kTime))
...
fig.update_layout(
    ...
    title={'text': f"TELLUS Pressure at Site {holeID} ({begin.year}.{begin.month:02d})", 'y':0.98, 'x':0.5, 'xanchor':'center', 'yanchor':'top'})
...
for idx, eqvline in kigam.iterrows():
    tY, tM, tD, tHH, tMM = eqvline.Time.year, eqvline.Time.month, eqvline.Time.day, eqvline.Time.hour+9, eqvline.Time.minute
    if tHH>=24:
        tHH = tHH - 24
        tD = tD + 1
    fig.add_vline(x=datetime.datetime(tY,tM,tD,tHH,tMM), line_color='gray', line_dash = 'dot', row='all', col=1)
...
if result_out == "mon":
    ...
    fname = f"D:\\TELLUS_PT_sensor_{holeID}_{begin.year}_{begin.month:02d}"
