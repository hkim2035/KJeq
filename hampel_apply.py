...
logfile = open("D:\\git\\data\\PT_A1\\hampel.log", 'w')
all_files = sorted(glob.glob("D:\\git\\data\\PT_A1\\*.csv"))

for raw in all_files:
    df = pd.read_csv(raw, sep=',', header=0, skip_blank_lines=True)
    df.rename(columns={"MULTI_P1_REAL": "P1000",
              "MULTI_P2_REAL": "P500"}, inplace=True)
    df.rename(columns={"MULTI_T1_REAL": "T1000",
              "MULTI_T2_REAL": "T500"}, inplace=True)
